...
| **Always a way out** | A no-injection mode (ETW-based) is the default for anything the software is unsure about. It needs the Agent running elevated, because Windows restricts the trace sessions it uses — FrameLedger tells you when that applies rather than silently recording less<br><br>⚠ **None of this is built, and as of 2026-08-27 it does not even have a chosen mechanism.** The design named Intel PresentMon; that was dropped after measurement, and no replacement has been picked (`docs/20_OPEN_QUESTIONS.md` §G). **Today a refused or unhookable title gets Tier 3** — session duration and hardware telemetry — not a degraded measurement. Said here rather than in a footnote, because this row is in the SAFETY table and a way out that does not exist is the wrong thing to be reassured by |
