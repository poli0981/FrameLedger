...
**Struct mirroring —** `FlFrameRecord` is **specified** to exist twice (C++ header, C# `[StructLayout(LayoutKind.Sequential)]`), with a test asserting `sizeof` and every field offset on both sides, and a version constant in the shm header that must match or the Agent refuses to attach.

**It exists once.** This paragraph was in the present tense until 2026-08-05 and was false: `src/FrameLedger.Shared/` holds a `.csproj` and no `.cs` files, `grep -rn FlFrameRecord src tests --include=*.cs` returns nothing, and `FL_BUILD_ID` — the input to the refuse-to-attach comparison — has a producer and no reader. `build.ps1:324-329` enumerates **nine** files that described the mechanism as live, including `fl_shm.h` (normative) and `fl-layout-dump/CMakeLists.txt` (a build file); `build.ps1:335-342` skips the gate loudly instead of pretending it runs. Tracked as `20_OPEN_QUESTIONS` §R10 and scheduled in §S24. **Until it lands, changing a field in `fl_shm.h` breaks the Agent at runtime rather than at build time** — treat the layout as frozen unless you are the one building the mirror. Same reading rule as §Dev mode below, and the same reason: a doc that says a gate runs is worse than one that says it is missing.
