...
> ⚠ **Accuracy audit — re-checked 2026-08-05, a THIRD time, after PRs #46–#52. SIX statements below describe behaviour the software does not yet fully have**, and they are flagged here rather than quietly reworded because a document the user accepts must not over-promise.
>
...
> **Today that list is one item long: presentation.** No upscaling, ray-tracing or pipeline hook exists yet, and video-memory usage is not read. The list describes what the software is designed to observe, and the boundary — API arguments and nothing else — holds for all of it. See the accuracy audit at the top.
...
> **Frame timing is measured today. The settings are not measured at all yet** — the hooks that would read those parameters do not exist. Where a value has not been measured the software marks it as unmeasured rather than reporting a zero, so it will read `N/A` and never a confident wrong answer. See the accuracy audit at the top.
