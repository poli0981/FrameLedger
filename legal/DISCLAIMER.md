...
> ⚠ **Accuracy audit — re-checked 2026-08-05 after PRs #40–#44. FOUR statements below describe behaviour the software does not yet have**, and they are flagged here rather than quietly reworded because a document the user accepts must not over-promise. **Two of them changed substance in this re-check** — the capture side that receives a stop now exists, while the side that would send one still does not:
>
> - §2's *"and every 30 seconds afterwards … it stops at the next scan"* — the pre-injection guard is real and refuses. The **receiving half is now implemented**: an injected D3D title removes its hooks within one frame of the stop flag being set. The **sending half is still missing** — `GuardSupervisor` has no production caller and nothing in production writes the shared-memory field it would advance, so no re-scan runs and no stop is ever sent. A half-built mechanism must not read as a whole one.
> - §2's *"waits **65 seconds** before concluding that contact is lost"* — **the deadline now has a reader** (`FrameLedger.Overlay`'s present path), which the previous audit correctly said it did not. Three qualifications, none of them cosmetic: it is evaluated **only when the game presents a frame**, so a title that has hung or been alt-tabbed never reaches it — the exact case the mechanism was specified for; it covers **injected Direct3D titles only**, because the Vulkan layer intercepts nothing yet; and **no test asserts that it fires**. Until those are fixed the sentence promises more than the software delivers.
...
>
> **This block is maintained by hand and nothing verifies it.** Every other document here is bound to the code by something — `rules-validate` cross-checks the blocklist, `static_assert`s bind `fl_shm.h` to `07_IPC`, `versioninfo-check` and `chokepoint-check` bind claims to binaries. `legal/` is bound by nothing, and it has now gone stale **twice**: first within hours of being written, when the 65-second sentence was added directly beneath a header that said "Three"; then again when five PRs (#40–#44) changed the Overlay from a stub into a hooking, shared-memory-mapping, control-block-reading component and touched **no documentation at all**, leaving the second bullet above asserting four things about that binary that were each false. Neither drift was visible from inside the PR that caused it. **Whoever edits any promise in this file must re-count — and whoever changes what `FrameLedger.Overlay` does must re-read this block, because nothing will remind them.**
