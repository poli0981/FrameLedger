...
| **PresentMon distribution** | `12_BUILD` says "bundled … pinned, SHA-256 verified at build" — is the binary committed to the repo or fetched at build time? Affects `.gitignore`, CI and the release package. **Still open, and now with one fact under it (2026-08-27):** a copy of `PresentMon-2.5.1-x64.exe` is staged locally for the §S31 run, hash-identical to the one pinned in `spike-notes` §11, and it was sitting **untracked and unignored at the repository root** where a `git add -A` would have committed it. `/presentmon/` is now ignored; `assets/native/PresentMon.exe` deliberately is **not**, so taking this decision is still one `git add` away rather than one `.gitignore` edit away. **Whichever way it goes, the pin can only mean HASH + FILENAME** — §M2 measured that the binary carries no VERSIONINFO at all, so there is nothing in it for a `versioninfo-check`-style gate to read. And committing it is not free: `license-check` binds `assets/native/PresentMon.exe` to the `Intel PresentMon` notices row **bidirectionally**, so the file appearing while the row still says "Not yet" is a red gate — correctly, because that is the moment the notice starts under-disclosing | `12_BUILD`, `THIRD_PARTY_NOTICES` |
