...
| — | **Multi-process titles are refused as `TargetAmbiguous` in v1** (measured 2026-09-03, *Flower in Us*, NW.js / RPG Maker) | Chromium-based runtimes — NW.js, Electron, RPG Maker MV/MZ, a large share of the VN catalogue — run several processes from ONE image path (browser, renderer, GPU). The resolver refuses rather than guess, which is §S27's consent-keyed-on-path rule working as written. The presenting process is Chromium's GPU process, which owns no window, so neither "the one with the window" nor "the parent" is a correct pick, and `--pid` stays forbidden. Reversing this needs a per-process discriminator that is not the path (command line `--type=gpu-process`, or the process that created the swapchain) and its own §S27 argument. Until then these titles are Tier 2 |
