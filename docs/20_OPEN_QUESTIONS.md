...
> | **Cyberpunk 2077**, FSR 3 + FG | FSR 3.0 host DLLs for upscaling, the **1.1.x monolith for frame generation** | `N/A` (upscale is `ffx_fsr3upscaler_x64.dll`, the deferred route) — render 1506×847 from PREPARE | `FsrFg` (4261 generated batches) | `72.93 → 145.76 (×2)` | — (no UPSCALE dispatch; count = Streamline tokens 4266) |
...
> | **Black Myth: Wukong**, FSR, FG off (×2 runs) | `N/A` | `none` (Streamline tokens = presents) | `FPS: 39.15 / 90.64` | — (no dispatch at the monolith OR the loader) | **not R6 at all** — see below |
>
...
> idle while FG is off.** The leg that tests it is Wukong with **FSR frame generation ON**: the
> Cyberpunk shape says what to expect — `FsrFg` from the monolith with `upscaler: N/A`. Owed;
> if that leg also reads silent, the monolith is loaded for nothing this hook can see and the
> title joins Rune Factory in §Scope decisions' static row.
>
