...
- **Record and header layout:** `static_assert(sizeof(...) == 64)` and `offsetof` assertions for `FlFrameRecord` **and all three header structs** (`FlShmHandshake`, `FlWriterState`, `FlControlBlock`) — the Agent reads across the process boundary in every one of them. The native half is done — `layout_test.cpp` plus the `static_assert`s in `fl_shm.h`, ctest `fl_shm_layout`. **The managed half is not: `tools/fl-layout-dump` emits the runtime offset dump and nothing consumes it** (`20_OPEN_QUESTIONS` §R10, scheduled in §S24). This clause read "Runtime offset dump consumed by the managed struct-mirror test" until 2026-08-05, which described a consumer that does not exist.
