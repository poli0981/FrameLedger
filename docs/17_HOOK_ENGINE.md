...
> **This table is a SPECIFICATION. As of 2026-09-04 the Overlay installs ELEVEN DETOURS —
> the eight of 2026-09-03 plus three `ffxDispatch` trampolines, one per AMD leaf — and the
> FidelityFX row below is the ninth ✅.** The counts differed until 2026-09-03, because a
...
> is the opposite shape — **one row-family, three modules, three patches from one
> template body** — because the three leaves export the same five names and each must be
> patched at its own address; and its family is published only once **every leaf the
